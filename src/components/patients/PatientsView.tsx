...
const PATIENT_CSV_HEADERS = ['first_name','last_name','age','sex','contact_number','treatment','allergies','medical_history']
const PATIENT_CSV_TEMPLATE = [
  PATIENT_CSV_HEADERS.join(','),
  'Juan,dela Cruz,34,male,09171234567,Teeth Cleaning,NKDA,',
  'Maria,Santos,39,female,09281234567,Root Canal Treatment,Penicillin,Hypertension',
].join('\n')
...
export default function PatientsView({ patients, total, initialSearch = '', initialTreatment = '' }: { patients: Patient[]; total: number; initialSearch?: string; initialTreatment?: string }) {
  const router = useRouter()
  const [showNew, setShowNew] = useState(false)
  const [search, setSearch] = useState(initialSearch)
  const [treatment, setTreatment] = useState(initialTreatment)

  function applyFilters(overrides: { search?: string; treatment?: string } = {}) {
    const s = overrides.search ?? search
    const t = overrides.treatment ?? treatment
    const params = new URLSearchParams()
    if (s) params.set('search', s)
    if (t) params.set('treatment', t)
    router.push(`/patients${params.size ? '?' + params.toString() : ''}`)
  }
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex gap-2 flex-wrap">
          <form onSubmit={handleSearch} className="flex items-center gap-2 bg-white border border-slate-200 rounded-lg px-3 py-2">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name…" className="text-sm outline-none w-36 text-slate-800 placeholder:text-slate-400" />
          </form>
          <form onSubmit={handleSearch} className="flex items-center gap-2 bg-white border border-slate-200 rounded-lg px-3 py-2">
            <Search size={14} className="text-slate-400" />
            <input
              value={treatment}
              onChange={e => { setTreatment(e.target.value); if (!e.target.value) applyFilters({ treatment: '' }) }}
              placeholder="Filter by treatment…"
              className="text-sm outline-none w-40 text-slate-800 placeholder:text-slate-400"
            />
            {treatment && (
              <button type="button" onClick={() => { setTreatment(''); applyFilters({ treatment: '' }) }} className="text-slate-400 hover:text-slate-600 text-xs leading-none">✕</button>
            )}
          </form>
        </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              {['Patient / ID','Age','Treatment','Contact','Balance','Status',''].map(h => (
            ...
            {patients.map(p => (
              <tr key={p.id} className="border-b border-slate-100 hover:bg-blue-50/50 transition-colors">
                <td className="px-4 py-3">
                  ...
                  <div className="text-xs text-slate-400 capitalize">{p.sex ?? '—'}</div>
                </td>
                <td className="px-4 py-3 text-sm text-slate-600">{p.age ?? '—'}</td>
                <td className="px-4 py-3">
                  {p.treatment
                    ? <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">{p.treatment}</span>
...
function SinglePatientForm({ onClose }: { onClose: () => void }) {
  ...
  const [f, setF] = useState({ first_name:'', last_name:'', age:'', sex:'female', contact_number:'', treatment:'', allergies:'', medical_history:'' })
  ...
  async function save() {
    ...
    const payload = {
      ...
      sex: f.sex, contact_number: f.contact_number || null,
      treatment: f.treatment || null,
      allergies: f.allergies || null, medical_history: f.medical_history || null,
      patient_type: 'dental', clinic_id: staff.clinic_id, source: 'walk_in',
    }

    const { data: patient, error } = await supabase.from('patients').insert(payload).select().single()
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <F label="Age" type="number" value={f.age} onChange={s('age')} placeholder="e.g. 34" />
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <F label="Treatment" value={f.treatment} onChange={s('treatment')} placeholder="e.g. Root Canal Treatment" />
      </div>
...
function BulkPatientUpload({ onClose }: { onClose: () => void }) {
  ...
  async function upload() {
    ...
    const payload = valid.map(r => ({
      ...
      age: r.age?.trim() ? parseInt(r.age.trim()) || null : null,
